"""
HEALSwin for Conditional Flow Matching
=============================================
Adapted from:
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, emb=None):
        ...
        Args:
            x:   (B, N, C)
            emb: (B, C) conditioning embedding
        """
        B, N, C = x.shape
        shortcut = x
    ...
    def forward(self, x, emb=None):
        for blk in self.blocks:
            if self.use_checkpoint:
                # Pass emb as an explicit positional arg — avoids the closure bug
                # where a lambda captures emb by reference and gets the wrong value
                # during backward recomputation with use_reentrant=False.
